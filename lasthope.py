...
"""
    ...
    for i, (genome_id1, genome1) in enumerate(genomes):
        ...
        for genome_id2, genome2 in genomes[i + 1:]:
            ...
            player1 = BetterPlayer("gen8ou", random.choice(teams),99, neat.nn.FeedForwardNetwork.create(genome1,config))
            player2 = BetterPlayer("gen8ou", random.choice(teams),99, neat.nn.FeedForwardNetwork.create(genome2,config))
            print("Running Match " + str(k) + " between genomes " + str(genome_id1) + " and " + str(genome_id2))
            asyncio.run(calculate_fitness(genome1, genome2, player1, player2))
            k += 1
    for i, (genome_id1, genome) in enumerate(genomes):
        genome.fitness = float(genome.fitness) / (len(genomes) - 1)
    winner = genomes[0][1]
    winning_fitness = winner.fitness
    for i, (genome_id1, genome) in enumerate(genomes):
        if genome.fitness > winning_fitness:
            winner = genome
            winning_fitness = genome.fitness
